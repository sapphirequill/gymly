...
#[AsMessageHandler]
final readonly class RemoveExerciseFromTrainingPlanHandler
{
    public function __construct(private TrainingPlanRepository $trainingPlanRepository)
    {
    }

    public function __invoke(RemoveExerciseFromTrainingPlanCommand $command): void
    {
        $trainingPlan = $this->trainingPlanRepository->getById($command->trainingPlanId);

        $trainingPlan->removeExercise($command->exerciseCode);
